...
echo "0;0;0;0;dummy;${threshold};${lenlimit}"
...
do
    ...
    mlr --headerless-csv-output --csv --ifs ',' --ofs ';' --from ${file} \
        cat -n \
        then filter "float(\$fitness) > ${threshold} && \$nodes <= ${lenlimit}" \
        then cut -f n,generation,individual_index,expression,fitness,nodes \
        then head -n 1
